#!/usr/bin/env node
/** Build Oberon's official Voyager-mosaicked appearance and explicit synthetic relief. */
...
const source = {
  file: "oberon-voyager-texture.webp",
  url: "https://assets.science.nasa.gov/dynamicimage/assets/science/cds/3d/resources/image/uranus---oberon/preview.webp?w=2048",
  sha256: "4579ddf4324669387024c0176543691d008543edac94c0c7e3195f6a65fd83b9",
...
async function verifySource() {
  ...
  if (actual !== source.sha256)
    throw new Error(`Oberon source SHA-256 mismatch: ${actual}`);
  ...
  if (
    ...
    throw new Error(
      `Unexpected Oberon source raster ${metadata.width}x${metadata.height}/${metadata.channels}ch`,
    );
  return path;
}
  ...
  for (let y = 0; y < height; y++)
    for (let x = 0; x < width; x++) {
      ...
      if (sample < 0 || sample > 65535)
        throw new Error(`Oberon procedural height exceeds uint16: ${sample}`);
...
async function rebuild() {
  ...
  for (const level of levels) {
    const colorPath = resolve(assets, `oberon-${level.name}.jpg`);
    await sharp(input)
      .resize(level.width, level.height, { fit: "fill", kernel: "lanczos3" })
      .jpeg({ quality: 88, chromaSubsampling: "4:4:4" })
      .toFile(colorPath);
    const entry = {
      color: `/assets/surfaces/oberon-${level.name}.jpg`,
      width: level.width,
      height: level.height,
    ...
    if (level.includeHeight) {
      ...
      const heightPath = resolve(assets, `oberon-${level.name}.height.bin`);
      await writeFile(heightPath, relief.bytes);
      Object.assign(entry, {
        heightUrl: `/assets/surfaces/oberon-${level.name}.height.bin`,
  ...
  const manifest = {
    schemaVersion: 1,
    generatedBy: "scripts/prepare-oberon-surface.mjs",
    body: "oberon",
    referenceRadiusMeters: 761400,
    bodyrefSIradius: 761400,
    ...
    coordinates: {
      longitude:
        "east-positive -180..180 (alignment unverified by source metadata)",
      latitude: "planetocentric (alignment unverified by source metadata)",
      projection: "equirectangular texture supplied for NASA 3-D resource",
      northAtTop: true,
      pixelRegistration: "pixel-centered",
      sourceCoverage:
        "Voyager 2 mapped approximately 40% of Oberon; unobserved regions are retained as source appearance gap and are not presented as measured coverage",
    },
    heightPolicy:
      "No reliable global Oberon DEM is available. Deterministic restrained relief is generated from a seamless 3D direction-vector field independent of color; it is an appearance approximation, not measured Oberon topography or image-brightness inference.",
    provenance: {
      citation:
        "NASA Uranus–Oberon 3-D resource, mosaicked with Voyager imagery; credit USGS/Tammy Becker and JPL/Caltech",
      source,
      sourceMetadata: "https://science.nasa.gov/3d-resources/uranus-oberon/",
      missionReference:
        "https://www.jpl.nasa.gov/images/pia00036-oberon-high-resolution-color-composite/",
      coverage:
        "Voyager 2 mapped approximately 40% of Oberon during the 1986 flyby; the official texture retains unmapped/black regions rather than inventing source imagery.",
      license:
      ...
      canonicalRadiusMeters: 761400,
      sourceReferenceRadiusMeters: null,
    },
    levels: levelsManifest,
  };
  await writeFile(
    resolve(assets, "oberon-manifest.json"),
...
async function main() {
  ...
  if (args.has("--download-pinned")) {
    ...
    console.log(
      "downloaded pinned Oberon Voyager texture; rebuild with --rebuild",
    );
    if (!args.has("--rebuild")) return;
  }
  if (!args.has("--rebuild")) {
    console.error(
      "Usage: node scripts/prepare-oberon-surface.mjs --download-pinned | --rebuild",
    );
    process.exitCode = 2;
    return;
  }
