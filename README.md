...
Axial orientation uses dimensionless unit quaternions and angular velocity in radians per second. The Sun and all eight planets use fixed J2000 pole directions from [JPL's IAU orientation kernel](https://naif.jpl.nasa.gov/pub/naif/generic_kernels/pck/pck00011.tpc), with rounded sidereal periods. The significant Mars/Neptune periodic pole terms are evaluated at J2000; small Jovian nutations and later pole precession are omitted. The shared equatorial-to-world conversion also constructs moon reference planes. Illustrative new bodies default to a 24-hour spin unless their preset name matches a canonical planet/star. Regular moons use an **idealized synchronous orientation**: one face follows the actual primary direction and the pole follows orbital angular momentum. Their stored period is updated from the actual angular rate, not a 24-hour placeholder. This intentionally omits lunar obliquity, libration and tidal torque evolution. A moon becoming unbound or losing its primary retains its last angular velocity as a free spin. Spin does not feed back into point-mass gravity. History records orientation, angular velocity and derived period without treating orbital changes as edits.

Angular velocity is a pseudovector: the reflected world coordinate system contributes one sign reversal, and retrograde spin another. Venus takes 243.018 Earth days per sidereal rotation; Uranus takes 17.24 hours. Their spin/orbit angles are approximately 177.36° and 97.77°. Uranus's regular moons orbit in the same sense as its spin, while Triton orbits opposite Neptune. Neptune uses the newer IAU rotation rate (about 15.9663 hours). Initial prime meridians remain illustrative, not an ephemeris. The Sun is represented by a single nominal spin rather than differential rotation.
...
Saturn's C/B/A rings are thin world-space annuli aligned with its rotating equator, with a Cassini-division gap. The rear half is drawn behind the planet and the front half in front, with near-plane and viewport clipping. Physical ring distances are in meters; unresolved rings disappear into the distant body marker. Rings remain decorative: no particle gravity, ring collisions, shadows or material dynamics are modeled.
...
- `rings.js`: equatorial ring geometry and clipped perspective rendering.
...
The subsequent all-planet/moon orientation audit and corrections, including Saturn's ring geometry, are recorded in [canonical-data-corrections.md](docs/canonical-data-corrections.md).
